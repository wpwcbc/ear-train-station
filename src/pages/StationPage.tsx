...
import {
  hasShownDailyGoalReachedToast,
  markDailyGoalReachedToastShown,
} from "../lib/dailyGoalToast";
import {
  loadIntervalMissDetails,
  loadIntervalMissHistogram,
  recordIntervalMiss,
  recordIntervalPracticeHit,
} from "../lib/intervalStats";
import {
  addMistake,
  loadMistakes,
...
import {
  playIntervalPrompt,
  playNoteSequence,
  playRootThenChordPrompt,
  playTonicTargetPrompt,
} from "../audio/prompts";
import {
...
function intervalMissList(stationId: StationId): {
  label: IntervalLabel;
  count: number;
  weight: number;
  lastMissAtMs: number;
  ...
  const rows = Array.from(counts.entries()).map(([label, count]) => {
    ...
    const decayFactor =
      lastMissAtMs > 0 ? Math.exp((-LN2 * age) / WEIGHT_DECAY_HALF_LIFE_MS) : 1;
    const weight = Math.max(0, Math.round(count * decayFactor));

    const recencyBoost =
      lastMissAtMs > 0 ? Math.exp(-age / RECENCY_BUMP_HALF_LIFE_MS) : 0; // 1 → 0
  ...
  return visible.map(({ label, count, weight, lastMissAtMs }) => ({
    label,
    count,
    weight,
    lastMissAtMs,
...
export function StationPage({
  ...
  const examSection = examSectionId
    ? (SECTIONS.find((s) => s.id === examSectionId) ?? null)
    : null;
  const examSectionIndex = examSectionId
    ? SECTIONS.findIndex((s) => s.id === examSectionId)
    : -1;
  const nextSection =
    examSectionIndex >= 0 ? (SECTIONS[examSectionIndex + 1] ?? null) : null;
  ...
  const [practiceFocusIntervals, setPracticeFocusIntervals] = useState<
    IntervalLabel[] | null
  >(null);
  ...
  const [practiceWeightedSemitones, setPracticeWeightedSemitones] = useState<
    number[] | null
  >(null);
  ...
  const practiceLeftExtras = practice ? (
    ...
    >
      ...
      {practiceFocusIntervals?.length || practiceWeightedSemitones?.length ? (
        <button
          className="ghost"
          onClick={clearPracticeFocus}
          title="Clear focused/targeted practice"
        >
          Clear focus
        </button>
      ) : null}
      ...
      <div style={{ marginTop: 8 }}>
        <div style={{ fontSize: 12, opacity: 0.85 }}>
          ...
          {top
            .map(
              (x) =>
                `${x.label}×${x.count}${x.lastMissAtMs ? ` (${ageShort(now, x.lastMissAtMs)})` : ""}`,
            )
            .join(" · ")}
        ...
        <details style={{ marginTop: 6 }}>
          ...
          >
            <button
              className="ghost"
              onClick={() => {
                const topLabels = allIntervalMisses
                  .slice(0, 5)
                  .map((x) => x.label);
  ...
  const chordMode: "block" | "arp" =
    id.startsWith("T") || id.startsWith("E") ? "block" : "arp";
  ...
  const intervalPromptModeLabel =
    intervalPromptMode === "harmonic" ? "Harmonic" : "Melodic";
  ...
  const isIntervalStation =
    id === "S3_INTERVALS" ||
    id === "T3B_INTERVALS" ||
    id === "T3_INTERVALS" ||
    id === "E3_INTERVALS";
  const showHarmonicTips =
    isIntervalStation && intervalPromptMode === "harmonic";
  ...
  function startIntervalTargetedMix() {
    ...
    const basis = (
      topIntervalMisses.length
        ? topIntervalMisses
        : allIntervalMisses.slice(0, 5)
    ).slice(0, 5);
    const weighted = basis.flatMap((x) =>
      Array(Math.min(6, Math.max(2, x.weight))).fill(
        LABEL_TO_SEMITONE[x.label],
      ),
    );
  ...
  const [s3DeriveResult, setS3DeriveResult] = useState<
    "idle" | "correct" | "wrong"
  >("idle");
  ...
  const s3DeriveQ = useMemo(
    () =>
      makeIntervalDeriveQuestion({
        seed: seed * 1000 + 3000 + s3DeriveIndex,
        choiceCount: 4,
      }),
    [seed, s3DeriveIndex],
  );
  ...
  const [highlighted, setHighlighted] = useState<
    Record<number, "correct" | "wrong" | "active">
  >({});
  ...
  const [t3bLastWrongChoice, setT3bLastWrongChoice] =
    useState<IntervalLabel | null>(null);
  useEffect(() => {
  ...
  const [t3LastWrongChoice, setT3LastWrongChoice] =
    useState<IntervalLabel | null>(null);
  useEffect(() => {
  ...
  const t3Q = useMemo(
    () =>
      makeIntervalLabelQuestion({
        ...
        registerPolicy: "wide",
        // Default wide max is fine here (keeps target <= C6 when +12)
        minSemitones: 0,
        maxSemitones: 12,
        allowedSemitones: practice ? practiceAllowedSemitones : undefined,
        choiceCount: 6,
  ...
  const [e3LastWrongChoice, setE3LastWrongChoice] =
    useState<IntervalLabel | null>(null);
  useEffect(() => {
  ...
  const triadQ = useMemo(
    () => makeTriadQualityQuestion({ seed: seed * 1000 + 4 }),
    [seed],
  );
  ...
  const diatonicQ = useMemo(
    () => makeDiatonicTriadQualityQuestion({ seed: seed * 1000 + 5 }),
    [seed],
  );
  ...
  const funcQ = useMemo(
    () => makeFunctionFamilyQuestion({ seed: seed * 1000 + 6 }),
    [seed],
  );
  ...
  const s1bTwistDone =
    s1bTwistIndex >= S1B_TWIST_TOTAL || s1bTwistWrong >= HEARTS;
  ...
  const s1cTwistDone =
    s1cTwistIndex >= S1C_TWIST_TOTAL || s1cTwistWrong >= HEARTS;
  ...
  const s2PatternQ = useMemo(
    () =>
      makeMajorScaleStepTypeQuestion({
        seed: seed * 1000 + 210,
        stepIndex: s2PatternIndex,
      }),
    [seed, s2PatternIndex],
  );

  const s2Session = useMemo(
    () => makeMajorScaleSession({ seed: seed * 1000 + 2 }),
    [seed],
  );
  ...
  const s2ShownSoFar = useMemo(
    () => s2Session.scale.slice(0, s2Step),
    [s2Session, s2Step],
  );
  ...
  const s2ScaleMidis = useMemo(
    () => MAJOR_OFFSETS.map((o) => s2Session.tonicMidi + o),
    [s2Session],
  );
  const s2ScaleSoFarMidis = useMemo(
    () => s2ScaleMidis.slice(0, s2Step),
    [s2ScaleMidis, s2Step],
  );

  function applySectionExamPass(
    p: Progress,
    passedStationId: StationId,
  ): Progress {
  ...
  async function chooseS3Twist(choice: IntervalLabel) {
    ...
    if (!ok) {
      ...
      trackMistake({
        kind: "intervalLabel",
        sourceStationId: id,
        rootMidi: s3TwistQ.rootMidi,
        semitones: s3TwistQ.semitones,
      });
  ...
  async function playPromptS1Twist() {
    ...
    await piano.playMidi(s1TwistQ.midi, {
      durationSec: dur(0.9),
      velocity: 0.95,
    });
    setHighlighted({});
  ...
  async function playPromptS1BTwist() {
    ...
    await piano.playMidi(s1bTwistQ.midi, {
      durationSec: dur(0.9),
      velocity: 0.95,
    });
    setHighlighted({});
  ...
  async function playPromptS1CTwist() {
    ...
    await piano.playMidi(s1cTwistQ.midi, {
      durationSec: dur(0.9),
      velocity: 0.95,
    });
    setHighlighted({});
  ...
  async function playS2Scale(kind: "soFar" | "full" | "fullOctave") {
    ...
    setHighlighted(
      Object.fromEntries(seq.map((m) => [m, "active"])) as Record<
        number,
        "active"
      >,
    );
    await playNoteSequence(seq, {
      durationSec: dur(0.45),
      velocity: 0.9,
      gapMs: gap(90),
    });
    setHighlighted({});
  ...
  async function playPromptS2() {
    ...
    await piano.playMidi(s2Q.tonicMidi, {
      durationSec: dur(0.65),
      velocity: 0.9,
    });
    await new Promise((r) => setTimeout(r, gap(250)));
    setHighlighted({ [s2Q.targetMidi]: "active" });
    await piano.playMidi(s2Q.targetMidi, {
      durationSec: dur(0.85),
      velocity: 0.9,
    });
    setHighlighted({});
  }
  ...
  async function chooseS2(choice: string) {
    ...
    if (s2Step >= 6) {
      ...
      rewardAndMaybeComplete(
        completionBonus,
        willHitGoal ? { stationDone: "S2_MAJOR_SCALE" } : undefined,
        { combo: false },
      );
  ...
  async function playPromptS4() {
    ...
    await piano.playMidi(triadQ.rootMidi, {
      durationSec: dur(0.65),
      velocity: 0.9,
    });
    await new Promise((r) => setTimeout(r, gap(250)));
    const active: Record<number, "active"> = Object.fromEntries(
      triadQ.chordMidis.map((m) => [m, "active"]),
    ) as Record<number, "active">;
    setHighlighted(active);
    await piano.playChord(triadQ.chordMidis, {
      mode: chordMode,
      durationSec: dur(1.1),
      velocity: 0.92,
      gapMs: gap(130),
  ...
  async function playPromptS5() {
    ...
    await new Promise((r) => setTimeout(r, gap(240)));
    const active: Record<number, "active"> = Object.fromEntries(
      diatonicQ.chordMidis.map((m) => [m, "active"]),
    ) as Record<number, "active">;
    setHighlighted(active);
    await piano.playChord(diatonicQ.chordMidis, {
      mode: chordMode,
      durationSec: dur(1.1),
      velocity: 0.92,
      gapMs: gap(130),
  ...
  async function playPromptS6() {
    ...
    await new Promise((r) => setTimeout(r, gap(240)));
    const active: Record<number, "active"> = Object.fromEntries(
      funcQ.chordMidis.map((m) => [m, "active"]),
    ) as Record<number, "active">;
    setHighlighted(active);
    await piano.playChord(funcQ.chordMidis, {
      mode: chordMode,
      durationSec: dur(1.1),
      velocity: 0.92,
      gapMs: gap(130),
  ...
  async function playKeyPrimerTriad(tonicMidi: number) {
    ...
    await piano.playMidi(tonicMidi + 4, {
      durationSec: dur(0.32),
      velocity: 0.86,
    });
    await new Promise((r) => setTimeout(r, gap(110)));
    setHighlighted({ [tonicMidi + 7]: "active" });
    await piano.playMidi(tonicMidi + 7, {
      durationSec: dur(0.32),
      velocity: 0.86,
    });
    await new Promise((r) => setTimeout(r, gap(110)));
    setHighlighted({ [tonicMidi + 12]: "active" });
    await piano.playMidi(tonicMidi + 12, {
      durationSec: dur(0.34),
      velocity: 0.86,
    });
  ...
  async function playPromptS8() {
    setResult("idle");
    setHighlighted({});
    await playTonicTargetPrompt(
      degreeIntervalQ.tonicMidi,
      degreeIntervalQ.targetMidi,
      {
        gapMs: gap(260),
  ...
  async function chooseT3B(choice: IntervalLabel) {
    ...
    if (!ok) {
      trackMistake({
        kind: "intervalLabel",
        sourceStationId: id,
        rootMidi: t3bQ.rootMidi,
        semitones: t3bQ.semitones,
      });
  ...
  async function chooseT3(choice: IntervalLabel) {
    ...
    if (!ok) {
      trackMistake({
        kind: "intervalLabel",
        sourceStationId: id,
        rootMidi: t3Q.rootMidi,
        semitones: t3Q.semitones,
      });
  ...
  async function chooseE3(choice: IntervalLabel) {
    ...
    if (!ok) {
      trackMistake({
        kind: "intervalLabel",
        sourceStationId: id,
        rootMidi: e3Q.rootMidi,
        semitones: e3Q.semitones,
      });
  ...
  async function chooseT8(choice: IntervalLabel) {
    ...
    if (!ok) {
      ...
      trackMistake({
        kind: "intervalLabel",
        sourceStationId: id,
        rootMidi: t8Q.tonicMidi,
        semitones: t8Q.semitones,
      });
  ...
  async function chooseS4(choice: "major" | "minor" | "diminished") {
    ...
    const correctHi: Record<number, "correct"> = Object.fromEntries(
      triadQ.chordMidis.map((m) => [m, "correct"]),
    ) as Record<number, "correct">;
    ...
    if (!ok) {
      ...
      trackMistake({
        kind: "triadQuality",
        sourceStationId: id,
        rootMidi: triadQ.rootMidi,
        quality: triadQ.quality,
      });
      setHighlighted({
        ...correctHi,
        ...(triadQ.chordMidis.includes(triadQ.rootMidi)
          ? {}
          : { [triadQ.rootMidi]: "correct" }),
      });
      return;
    }
  ...
  async function chooseS5(choice: "major" | "minor" | "diminished") {
    ...
    const correctHi: Record<number, "correct"> = Object.fromEntries(
      diatonicQ.chordMidis.map((m) => [m, "correct"]),
    ) as Record<number, "correct">;
    ...
    if (!ok) {
      ...
      trackMistake({
        kind: "triadQuality",
        sourceStationId: id,
        rootMidi: diatonicQ.chordMidis[0],
        quality: diatonicQ.quality,
      });
      setHighlighted(correctHi);
      return;
    }
  ...
  async function chooseS6(choice: FunctionFamily) {
    ...
    const correctHi: Record<number, "correct"> = Object.fromEntries(
      funcQ.chordMidis.map((m) => [m, "correct"]),
    ) as Record<number, "correct">;
    ...
    focus.setTopBar({
      ...
      badge: showChordBadge
        ? {
            ...
            title:
              "Pedagogy: lessons use broken chords (arpeggios) for clarity; tests/exams use block chords.",
          }
        : undefined,
      progress: progress01,
      hearts,
  ...
  return (
    <div className="focusStage">
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ opacity: 0.92 }}>
            Harmonic intervals are harder because both notes arrive at once —
            your brain doesn’t get the “jump” cue from melodic motion. Try these
            trainer moves:
          </div>

          <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 1.5 }}>
            <li>
              <b>Split it:</b> after the chord, sing/hum either the top or
              bottom note (even badly).
            </li>
            <li>
              <b>Arpeggiate mentally:</b> imagine root → target (or replay with
              helper on) to recover the melodic shape.
            </li>
            <li>
              <b>Listen for “color” first:</b> consonant vs crunchy, then refine
              (e.g. 4th vs 5th, 3rd vs 6th).
            </li>
            <li>
          ...
          <div style={{ fontSize: 12, opacity: 0.78, lineHeight: 1.45 }}>
            Quick reads:
            <a
              href="https://www.musical-u.com/learn/how-can-i-improve-at-harmonic-intervals/"
              target="_blank"
              rel="noreferrer"
            >
              Musical U
            ...
            {" · "}
            <a
              href="https://music.stackexchange.com/questions/59145/how-to-hear-lowest-note-in-harmonic-intervals"
              target="_blank"
              rel="noreferrer"
            >
              Music.SE thread
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button
              className="primary"
              onClick={() => setHarmonicTipsOpen(false)}
            >
              Got it
            </button>
      ...
      <div className="focusCenter">
        ...
        <div className="focusMetaRow">
          {stationMistakeCount > 0 ? (
            <Link
              ...
              className={
                stationMistakeDue > 0 ? "linkBtn primaryLink" : "linkBtn"
              }
              style={{ fontSize: 12, padding: "6px 10px" }}
            >
              Review
              {stationMistakeDue > 0
                ? ` (${stationMistakeDue} due)`
                : ` (${stationMistakeCount})`}
        ...
        >
          <div>
            Hotkeys: Space/Enter = Play/Hear • 1–9 = Answer • Backspace =
            Next/Restart
          </div>
          <button
            className="ghost"
        ...
        {done ? (
          ...
          >
            <div>
              ...
              {mistakesThisVisit > 0 ? (
                <div style={{ fontSize: 12, opacity: 0.9, marginTop: 6 }}>
                  Captured {mistakesThisVisit} mistake
                  {mistakesThisVisit === 1 ? "" : "s"} this visit — use Review
                  to clear them.
                </div>
              ) : null}
            </div>
            ...
            >
              ...
              {nextId && nextUnlocked ? (
                <Link className="linkBtn" to={`/lesson/${nextId}`}>
                  Next
                </Link>
              ) : null}
            </div>
          ...
          id === "S1_NOTES" ? (
            <>
              <DuoBottomBar
                ...
                right={
                  <span className="btnPrimary" style={{ opacity: 0.55 }}>
                    Check
                  </span>
                }
              />
              <TTTRunner
                    ...
                    pages={[
                      {
                        ...
                        body: "We’ll start with the white keys only: C D E F G A B.\n\nIn this lesson: stable register, one octave, no tricks. You’re training *recognition*, not speed.",
                        footnote:
                          "Hotkey: Space/Enter = play · 1–9 = answer · Backspace = next/restart",
                      },
                      {
                        title: "Anchor: Middle C",
                        body: "Middle C is your anchor. Once you can find C, the rest is just alphabet order.\n\nC → D → E (up), then F G A B.",
                      },
                      {
                        title: "Accidentals later",
                        body: "If you see black keys later: they can be sharp or flat.\nFor now, just nail the white-key names cleanly.",
                      },
                    ]}
                ...
                test={
                  <>
                    <HintOverlay
                      open={s1HintOpen}
                      onClose={() => setS1HintOpen(false)}
                    >
                      <ul style={{ margin: 0, paddingLeft: 18 }}>
                        <li>
                          Find <b>C</b> first. It’s the note just left of the
                          two-black-keys group.
                        </li>
                        <li>
                          From C, the white keys go up alphabetically: C D E F G
                          A B.
                        </li>
                        <li>
                          If you’re unsure, play it again — hearing twice is
                          normal.
                        </li>
                      </ul>
                    </HintOverlay>
                    ...
                    <div className={`result r_${result}`}>
                      ...
                      {result === "correct" &&
                        `Correct — +2 XP. (${noteQ.promptLabel})`}
                      {result === "wrong" &&
                        `Not quite — it was ${noteQ.promptLabel}.`}
                    </div>
                    ...
                    >
                      <StaffNote
                        midi={noteQ.midi}
                        spelling={noteQ.displaySpelling}
                        showLegend={false}
                      />
                    ...
                    {s1TestComplete ? (
                      ...
                      >
                        Nice — now we do a short <b>Twist</b> (scored, hearts
                        apply).
                      </div>
                    ) : null}
                  </>
                ...
                twist={
                  <>
                    ...
                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      ...
                      >
                        Q: {Math.min(s1TwistIndex + 1, S1_TWIST_TOTAL)}/
                        {S1_TWIST_TOTAL} · Correct: {s1TwistCorrect}/
                        {S1_TWIST_TOTAL} (need {S1_TWIST_PASS}) · Lives:{" "}
                        {Math.max(0, HEARTS - s1TwistWrong)}/{HEARTS}
                      </div>
                    </div>
                    ...
                    <div className={`result r_${result}`}>
                      ...
                      {result === "correct" &&
                        `Correct — +3 XP. (${s1TwistQ.promptLabel})`}
                      {result === "wrong" &&
                        (lessonRetryKey === `${id}:S1_TWIST:${s1TwistIndex}`
                          ? "Not quite — try once more."
                          : `Not quite — it was ${s1TwistQ.promptLabel}.`)}
                    </div>

                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      <ChoiceGrid
                        choices={s1TwistQ.choices}
                        onChoose={chooseS1Twist}
                      />
                    </div>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <StaffNote
                        midi={s1TwistQ.midi}
                        spelling={s1TwistQ.displaySpelling}
                        showLegend={false}
                      />
                    </div>
              ...
              <TTTRunner
                ...
                test={
                  <>
                    <HintOverlay
                      open={s1bHintOpen}
                      onClose={() => setS1bHintOpen(false)}
                      title="Staff hint"
                    >
                      <ul style={{ margin: 0, paddingLeft: 18 }}>
                        <li>
                          <b>Middle C</b> is your anchor. Step up/down by
                          letters.
                        </li>
                        <li>
                          If you see a note on a line, the next space up is the
                          next letter.
                        </li>
                    ...
                    <div className={`result r_${result}`}>
                      ...
                      {result === "correct" &&
                        `Correct — +2 XP. (${s1bQ.promptLabel})`}
                      {result === "wrong" &&
                        `Not quite — it was ${s1bQ.promptLabel}.`}
                    </div>
                ...
                twist={
                  <>
                    ...
                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      ...
                      >
                        Q: {Math.min(s1bTwistIndex + 1, S1B_TWIST_TOTAL)}/
                        {S1B_TWIST_TOTAL} · Correct: {s1bTwistCorrect}/
                        {S1B_TWIST_TOTAL} (need {S1B_TWIST_PASS}) · Lives:{" "}
                        {Math.max(0, HEARTS - s1bTwistWrong)}/{HEARTS}
                      </div>
                    </div>
                    ...
                    <div className={`result r_${result}`}>
                      {result === "idle" &&
                        (s1bTwistDone
                          ...
                          : "Twist: 10 questions. Need 8/10 to pass.")}
                      {result === "correct" &&
                        `Correct — +3 XP. (${s1bTwistQ.promptLabel})`}
                      {result === "wrong" &&
                        (lessonRetryKey === `${id}:S1B_TWIST:${s1bTwistIndex}`
                          ? "Not quite — try once more."
                          : `Not quite — it was ${s1bTwistQ.promptLabel}.`)}
                    </div>

                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      <ChoiceGrid
                        choices={s1bTwistQ.choices}
                        onChoose={chooseS1BTwist}
                      />
                    </div>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <StaffNote
                        midi={s1bTwistQ.midi}
                        spelling={s1bTwistQ.displaySpelling}
                        showLegend={false}
                      />
                    </div>
...
Context (sharp vs flat) depends on the key — we’ll cover that later. For now, both spellings are accepted.`,
                ...
                test={
                  <>
                    <HintOverlay
                      open={s1cHintOpen}
                      onClose={() => setS1cHintOpen(false)}
                      title="Accidentals hint"
                    >
                      <ul style={{ margin: 0, paddingLeft: 18 }}>
                        <li>
                          In the 2-black-keys group: left is <b>C# / Db</b>,
                          right is <b>D# / Eb</b>.
                        </li>
                        <li>
                          In the 3-black-keys group: left→right is{" "}
                          <b>F# / Gb</b>, <b>G# / Ab</b>, <b>A# / Bb</b>.
                        </li>
                        <li>
                          Both names are correct. Don’t overthink — recognize
                          fast.
                        </li>
                      </ul>
                    </HintOverlay>
                    ...
                    <div className={`result r_${result}`}>
                      ...
                      {result === "correct" &&
                        `Correct — +2 XP. (${s1cQ.promptLabel})`}
                      {result === "wrong" &&
                        `Not quite — it was ${s1cQ.promptLabel}.`}
                    </div>
                ...
                twist={
                  <>
                    ...
                    >
                      <ul style={{ margin: 0, paddingLeft: 18 }}>
                        <li>
                          2-black group: <b>C#</b>, <b>D#</b>.
                        </li>
                        <li>
                          3-black group: <b>F#</b>, <b>G#</b>, <b>A#</b>.
                        </li>
                    ...
                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      ...
                      >
                        Q: {Math.min(s1cTwistIndex + 1, S1C_TWIST_TOTAL)}/
                        {S1C_TWIST_TOTAL} · Correct: {s1cTwistCorrect}/
                        {S1C_TWIST_TOTAL} (need {S1C_TWIST_PASS}) · Lives:{" "}
                        {Math.max(0, HEARTS - s1cTwistWrong)}/{HEARTS}
                      </div>
                    </div>
                    ...
                    <div className={`result r_${result}`}>
                      {result === "idle" &&
                        (s1cTwistDone
                          ...
                          : "Twist: 10 questions. Need 8/10 to pass.")}
                      {result === "correct" &&
                        `Correct — +3 XP. (${s1cTwistQ.promptLabel})`}
                      {result === "wrong" &&
                        (lessonRetryKey === `${id}:S1C_TWIST:${s1cTwistIndex}`
                          ? "Not quite — try once more."
                          : `Not quite — it was ${s1cTwistQ.promptLabel}.`)}
                    </div>

                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      <ChoiceGrid
                        choices={s1cTwistQ.choices}
                        onChoose={chooseS1CTwist}
                      />
                    </div>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <StaffNote
                        midi={s1cTwistQ.midi}
                        spelling={s1cTwistQ.displaySpelling}
                        showLegend={false}
                      />
                    </div>
            ...
            <>
              <HintOverlay
                open={t1bHintOpen}
                onClose={() => setT1bHintOpen(false)}
                title="Hint"
              >
                <ul style={{ margin: 0, paddingLeft: 18 }}>
                  <li>
                    Find <b>Middle C</b> first. On piano: just left of the
                    2-black-keys group.
                  </li>
                  <li>
                    Then it’s just alphabet order on white keys:{" "}
                    <b>C D E F G A B</b>.
                  </li>
                  <li>
                    This mid-test has <b>no hearts</b> — play again as needed.
                  </li>
                </ul>
              </HintOverlay>
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Mid-test: stable register. Need 6/8 to pass."}
                {result === "correct" &&
                  (progress.stationDone["T1B_NOTES"]
                    ? "Passed — nice. (+10 bonus XP)"
                    : `Correct — +3 XP. (${t1bQ.promptLabel})`)}
                {result === "wrong" &&
                  (t1bDone
                    ? "Done — try Restart to improve your score."
                    : `Not quite — it was ${t1bQ.promptLabel}.`)}
              </div>
            ...
            <>
              <TestHeader
                ...
                onRestart={resetT1}
                reviewHref={
                  (t1Index >= T1_TOTAL || t1Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Test: name 10 notes (wider range). Need 8/10 to pass."}
                {result === "correct" &&
              ...
              <div style={{ marginTop: 10 }}>
                <StaffNote
                  midi={t1Q.midi}
                  spelling={t1Q.displaySpelling}
                  showLegend={false}
                />
              </div>

              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Tip: tests can roam across a bigger register; lessons stay in a
                stable register.
              </div>
            </>
          ) : id === "E1_NOTES" ? (
            <>
              <TestHeader
                ...
                onRestart={resetE1}
                reviewHref={
                  (e1Index >= E1_TOTAL || e1Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Section exam: 10 mixed notes (staff + accidentals, wider range). Need 8/10 to pass."}
                {result === "correct" &&
              ...
              <div style={{ marginTop: 10 }}>
                <StaffNote
                  midi={e1Q.midi}
                  spelling={e1Q.displaySpelling}
                  showLegend={false}
                />
              </div>

              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Duolingo-ish: pass the exam to "test out" and mark the whole
                section complete.
              </div>
            </>
          ) : id === "T2_MAJOR_SCALE" ? (
            <>
              <TestHeader
                ...
                onRestart={resetT2}
                reviewHref={
                  (t2Index >= T2_TOTAL || t2Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  (progress.stationDone["T2_MAJOR_SCALE"]
                    ? "Passed — nice. (+12 bonus XP)"
                    : "Correct — +3 XP.")}
                {result === "wrong" &&
                  (t2Wrong >= HEARTS
              ...
              <div style={{ marginTop: 10 }}>
                <PianoKeyboard
                  ...
                  octaves={4}
                  onPress={(m) =>
                    piano.playMidi(m, { durationSec: dur(0.9), velocity: 0.9 })
                  }
                  highlighted={highlighted}
                />
              </div>
            ...
            <>
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Mid-test: 8 interval labels. No hearts. Need 6/8 to pass."}
                {result === "correct" &&
                ...
                {result === "wrong" &&
                  (t3bDone
                    ? "Done — hit Restart to improve your score."
                    : `Not quite — it was ${t3bQ.correct} (${intervalLongName(t3bQ.correct)}).`)}
              </div>
              ...
              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  ...
                  getButtonClassName={(c) =>
                    `secondary${c === t3bLastWrongChoice ? " choiceWrong" : ""}`
                  }
                />
              </div>

              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Checkpoint vibe: play again anytime — no hearts here. If you
                miss, the app replays the correct interval once.
              </div>
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => {
                          setPractice(true);
                          setPracticeWeightedSemitones(null);
                          setPracticeFocusIntervals(
                            topIntervalMisses.map((x) => x.label),
                          );
                          resetT3B();
                        }}
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setPractice(true);
                          setPracticeFocusIntervals(null);
                          setPracticeWeightedSemitones(
                            weighted.length ? weighted : null,
                          );
                          resetT3B();
                        }}
            ...
            <>
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  (progress.stationDone["T3_INTERVALS"]
                    ? "Passed — nice. (+12 bonus XP)"
                    : `Correct — +3 XP. (${intervalLongName(t3Q.correct)})`)}
                {result === "wrong" &&
                  (t3Wrong >= HEARTS
                    ? `Out of lives. Score so far: ${t3Correct}/${T3_TOTAL}.` +
                      (stationMistakeCount > 0
                        ? " Review your misses, then restart."
                        : " Hit restart to try again.")
                    : t3Index + 1 >= T3_TOTAL
                      ? `Finished: ${t3Correct}/${T3_TOTAL}. Need ${T3_PASS}.` +
                        (stationMistakeCount > 0
                          ? " Review mistakes, then restart."
                          : " Hit restart to try again.")
              ...
              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  ...
                  disabled={correctionReplayBusy}
                  getButtonClassName={(c) =>
                    `secondary${c === t3LastWrongChoice ? " choiceWrong" : ""}`
                  }
                />
              </div>
              ...
              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Misses auto-replay the correct interval once (fast correction
                loop).
              </div>
            ...
            <>
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "wrong" &&
                  (e3Wrong >= HEARTS
                    ? `Out of lives. Score so far: ${e3Correct}/${E3_TOTAL}.` +
                      (stationMistakeCount > 0
                        ? " Review your misses, then restart."
                        : " Hit restart to try again.")
                    : e3Index + 1 >= E3_TOTAL
                      ? `Finished: ${e3Correct}/${E3_TOTAL}. Need ${E3_PASS}.` +
                        (stationMistakeCount > 0
                          ? " Review mistakes, then restart."
                          : " Hit restart to try again.")
              ...
              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  ...
                  disabled={correctionReplayBusy}
                  getButtonClassName={(c) =>
                    `secondary${c === e3LastWrongChoice ? " choiceWrong" : ""}`
                  }
                />
              </div>

              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Section exam: hearts on; pass = test out. Misses auto-replay the
                correct interval once.
              </div>
              ...
              {progress.stationDone[id] && examSectionId ? (
                <div className="callout" style={{ marginTop: 10 }}>
                  ...
                  <div style={{ marginTop: 4, opacity: 0.85 }}>
                    {examSection
                      ? `You cleared ${examSection.title}.`
                      : "Nice work — you cleared this section."}
                  </div>
                  <div
                    style={{
                      marginTop: 10,
                  ...
                  >
                    <Link
                      ...
                      to={`/learn/section/${examSectionId}`}
                    >
                      Back to section
                    </Link>
                    <Link
                      className="linkBtn"
                      to={`/learn/section/${examSectionId}/exam`}
                    >
                      Exam page
                    </Link>
                    {nextSection ? (
                      <Link
                        className="linkBtn"
                        to={`/learn/section/${nextSection.id}`}
                      >
            ...
            <>
              <div className="row">
                <button
                  className="primary"
                  aria-keyshortcuts="Space Enter"
                  onClick={playPromptS2}
                  disabled={!s2PatternDone}
                >
                  Hear target step
                </button>
                <button
                  className="secondary"
                  onClick={() =>
                    piano.playMidi(s2Session.tonicMidi, {
                      durationSec: dur(0.75),
                      velocity: 0.9,
                    })
                  }
                >
                ...
                <button
                  className="secondary"
                  onClick={() => void playS2Scale("soFar")}
                  disabled={!s2PatternDone}
                >
                  Scale so far
                </button>
                <button
                  className="ghost"
                  onClick={() => void playS2Scale("fullOctave")}
                >
                  Full scale
                </button>
                <button
                ...
                >
                  Scales: {Math.min(s2CompletedScales, S2_GOAL_SCALES)}/
                  {S2_GOAL_SCALES}
                </div>
              </div>
              ...
              {!s2PatternDone ? (
                <>
                  <div className={`result r_${result}`}>
                    ...
                    {result === "wrong" &&
                      "Not quite — try again. (Remember: W W H W W W H)"}
                  </div>
                  ...
                  >
                    <ChoiceGrid
                      choices={s2PatternQ.choices}
                      onChoose={chooseS2Pattern}
                    />
                  </div>
                ...
                <>
                  <div className={`result r_${result}`}>
                    ...
                    {result === "wrong" &&
                      `Not quite — next note is ${s2Q.correct}.`}
                  </div>
                    ...
                    pages={[
                      {
                        ...
                        body: "Goal: hear the distance between two notes and name it.\n\nLesson rules: stable register. No gotchas — just clean recognition.",
                        footnote:
                          "Tip: don't guess fast. Listen twice is normal.",
                      },
                      {
                  ...
                  <>
                    <div style={{ marginTop: 6, marginBottom: 10 }}>
                      ...
                      >
                        ...
                        <div style={{ fontSize: 12, opacity: 0.85 }}>
                          Progress: {Math.min(s3DeriveCorrect, S3_DERIVE_GOAL)}/
                          {S3_DERIVE_GOAL}
                        </div>
                      </div>
                      <div
                        className={`result r_${s3DeriveResult}`}
                        style={{ marginTop: 8 }}
                      >
                        {s3DeriveResult === "idle" && s3DeriveQ.prompt}
                        {s3DeriveResult === "correct" && "Correct — +1 XP."}
                        {s3DeriveResult === "wrong" &&
                          `Not quite — it was ${s3DeriveQ.correct}.`}
                      </div>
                      <div
                        className="row"
                      ...
                      >
                        <ChoiceGrid
                          choices={s3DeriveQ.choices}
                          onChoose={chooseS3Derive}
                          renderChoice={intervalLongName}
                        />
                      </div>
                      <div
                        style={{ fontSize: 12, opacity: 0.75, marginTop: 6 }}
                      >
                        Hint: minor = major − 1 semitone. (Perfect ± 1 is “the
                        weird ones”.)
                      </div>
                    </div>
                    ...
                    <div className="row">
                      ...
                      <button
                        ...
                        disabled={!s3WarmupDone}
                        onClick={() =>
                          piano.playMidi(intervalQ.rootMidi, {
                            durationSec: dur(0.9),
                          })
                        }
                      >
                ...
                twist={
                  <>
                    ...
                    >
                      ...
                      >
                        Q: {Math.min(s3TwistIndex + 1, S3_TWIST_TOTAL)}/
                        {S3_TWIST_TOTAL} · Correct: {s3TwistCorrect}/
                        {S3_TWIST_TOTAL} (need {S3_TWIST_PASS}) · Lives:{" "}
                        {Math.max(0, HEARTS - s3TwistWrong)}/{HEARTS}
                      </div>
                    </div>
                    ...
                    <div className={`result r_${result}`}>
                      {result === "idle" &&
                        (s3TwistDone
                          ...
                          : "Twist: 10 questions. Need 8/10 to pass.")}
                      {result === "correct" &&
                        `Correct — +3 XP. (${intervalLongName(s3TwistQ.correct)})`}
                      {result === "wrong" &&
                        (lessonRetryKey === `${id}:S3_TWIST:${s3TwistIndex}`
                          ? "Not quite — try once more."
                          : `Not quite — it was ${s3TwistQ.correct} (${intervalLongName(s3TwistQ.correct)}).`)}
                    </div>

                    <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                      <ChoiceGrid
                        choices={s3TwistQ.choices}
                        onChoose={chooseS3Twist}
                        renderChoice={intervalLongName}
                      />
                    </div>
            ...
            <>
              <div className="row">
                ...
                <button
                  className="secondary"
                  onClick={() =>
                    piano.playMidi(triadQ.rootMidi, {
                      durationSec: dur(0.8),
                      velocity: 0.9,
                    })
                  }
                ...
                {chordMode === "arp" ? (
                  <button
                    className="secondary"
                    onClick={playPromptS4BlockPreview}
                    title="Preview how tests/exams will sound"
                  >
                    Block
                  </button>
                ) : null}
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "wrong" &&
                  `Not quite — it was ${triadQualityLabel(triadQ.quality)} (${triadQualityIntervals(triadQ.quality).join("-")} semitones).`}
              </div>

              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  choices={triadQ.choices}
                  onChoose={chooseS4}
                  renderChoice={triadQualityLabel}
                />
              </div>
            ...
            <>
              <TestHeader
                ...
                onRestart={resetT5}
                reviewHref={
                  (t5Index >= T5_TOTAL || t5Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Test: name 10 triad qualities by ear. Need 8/10 to pass."}
                {result === "correct" &&
                  (progress.stationDone["T5_TRIADS"]
                    ? "Passed — nice. (+12 bonus XP)"
                    : `Correct — +3 XP. (${triadQualityLabel(t5Q.quality)})`)}
                {result === "wrong" &&
                  (t5Wrong >= HEARTS
            ...
            <>
              <div className="row">
                ...
                {chordMode === "arp" ? (
                  <button
                    className="secondary"
                    onClick={playPromptS5BlockPreview}
                    title="Preview how tests/exams will sound"
                  >
                    Block
                  </button>
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  `Correct — +5 XP. (${diatonicQ.key} major, degree ${diatonicQ.degree})`}
                {result === "wrong" &&
                  `Not quite — it was ${triadQualityLabel(diatonicQ.quality)}.`}
              </div>

              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  choices={diatonicQ.choices}
                  onChoose={chooseS5}
                  renderChoice={triadQualityLabel}
                />
              </div>
              ...
              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Hint: diatonic triad qualities in major are always: I ii iii IV
                V vi vii°.
              </div>
            ...
            <>
              <TestHeader
                ...
                onRestart={resetT6}
                reviewHref={
                  (t6Index >= T6_TOTAL || t6Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Test: identify diatonic triad quality in key by ear. Need 8/10 to pass."}
                {result === "correct" &&
            ...
            <>
              <div className="row">
                ...
                {chordMode === "arp" ? (
                  <button
                    className="secondary"
                    onClick={playPromptS6BlockPreview}
                    title="Preview how tests/exams will sound"
                  >
                    Block
                  </button>
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  `Correct — +5 XP. (${funcQ.key} major, ${funcQ.roman})`}
                {result === "wrong" &&
                  `Not quite — ${funcQ.roman} is ${familyLabel(funcQ.family)}.`}
              </div>

              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  choices={funcQ.choices}
                  onChoose={chooseS6}
                  renderChoice={familyLabel}
                />
              </div>

              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Quick map (major key): tonic = I iii vi · subdominant = ii IV ·
                dominant = V vii°.
              </div>
            ...
            <>
              <TestHeader
                ...
                onRestart={resetT7}
                reviewHref={
                  (t7Index >= T7_TOTAL || t7Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                {result === "idle" &&
                  "Test: name function family (tonic / subdominant / dominant). Need 8/10 to pass."}
                {result === "correct" &&
                  (progress.stationDone["T7_FUNCTIONS"]
                    ? "Passed — nice. (+12 bonus XP)"
                    : `Correct — +3 XP. (${t7Q.family})`)}
                {result === "wrong" &&
                  (t7Wrong >= HEARTS
            ...
            <>
              <div className="row">
                <button
                  className="primary"
                  aria-keyshortcuts="Space Enter"
                  onClick={playPromptS7}
                >
                  Hear degree
                </button>
                <button
                  className="ghost"
                  onClick={playKeyHintS7}
                  title="Hint: hear the key (do–mi–sol–do)"
                >
                  Hear key
                </button>
                <button
              ...
              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Cheat sheet: 1 tonic · 2 supertonic · 3 mediant · 4 subdominant
            ...
            <>
              <TestHeader
                ...
                onRestart={resetT4}
                reviewHref={
                  (t4Index >= T4_TOTAL || t4Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  (progress.stationDone["T4_DEGREES"]
                    ? "Passed — nice. (+12 bonus XP)"
                    : "Correct — +3 XP.")}
                {result === "wrong" &&
                  (t4Wrong >= HEARTS
            ...
            <>
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  `Correct — +3 XP. (${degreeIntervalQ.correct})`}
                {result === "wrong" &&
                  `Not quite — it was ${degreeIntervalQ.correct}.`}
              </div>

              <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
                <ChoiceGrid
                  choices={degreeIntervalQ.choices}
                  onChoose={chooseS8}
                />
              </div>

              <div style={{ fontSize: 12, opacity: 0.8, marginTop: 10 }}>
                Major scale intervals: 1=P1 · 2=M2 · 3=M3 · 4=P4 · 5=P5 · 6=M6 ·
                7=M7
              </div>
            </>
            ...
            <>
              <TestHeader
                ...
                onRestart={resetT8}
                reviewHref={
                  (t8Index >= T8_TOTAL || t8Wrong >= HEARTS) &&
                  stationMistakeCount > 0
                    ? `/review?station=${id}`
                    : undefined
                }
              ...
              <div className={`result r_${result}`}>
                ...
                {result === "correct" &&
                  (progress.stationDone["T8_DEGREE_INTERVALS"]
                    ? "Passed — nice. (+12 bonus XP)"
                    : `Correct — +3 XP. (${t8Q.correct})`)}
                {result === "wrong" &&
                  (t8Wrong >= HEARTS
          ...
          >
            <div style={{ minWidth: 220 }}>
              ...
              <div style={{ fontSize: 12, opacity: 0.9, marginTop: 4 }}>
                Today: <b>{Math.max(0, progress.dailyXpToday || 0)}</b>/
                <b>{Math.max(1, progress.dailyGoalXp || 0)}</b> XP · XP streak:{" "}
                <b>{progress.streakDays}</b> day
                {progress.streakDays === 1 ? "" : "s"} · Total XP:{" "}
                <b>{progress.xp}</b>
              </div>
